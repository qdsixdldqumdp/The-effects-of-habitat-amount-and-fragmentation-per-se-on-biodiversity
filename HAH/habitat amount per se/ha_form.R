...
inisimu<-function(nspn,nmapx,nmapy,envir){
  ...
  for (n in 1:spn) {
    mspp1 <- nlm_gaussianfield(mapx,mapy,resolution =1, autocorr_range =envir,mag_var = 2, nug = 0.1, mean = 0.5, rescale = TRUE)
...
fragmap<-function(patches){
  jihedis<-matrix(nrow=mapy,ncol=mapx)
  patchesvec<- c(1,2,4,8,16,36,72)
  patchesvec2<-c(1,1,2,2, 4,6, 6)
  patchesvec3<-c(1,2,2,4, 4,6,12)
  patchmat<-matrix(nrow = length(patchesvec),ncol = 3)
  patchmat[,1]<-patchesvec
  patchmat[,2]<-patchesvec2
  patchmat[,3]<-patchesvec3
  patchmatind<-which(patches==patchesvec)
  ivec<-patchmat[patchmatind,2]
  jvec<-patchmat[patchmatind,3]
  patchesnumber<-1
  for(i in 1:ivec){
    for(j in 1:jvec){
      i1<-((i-1)/ivec)*mapx+1
      ...
      j1<-((j-1)/jvec)*mapy+1
      j2<-(j/jvec)*mapy
      jihedis[i1:i2,j1:j2]<-patchesnumber
      patchesnumber<-patchesnumber+1
    }
  }
  
  jihepos<-matrix(0,nrow = patches,ncol = patches)
  #jihepos[ 1 , 2 ]<- 	0
  #jihepos[ 1 , 3 ]<- 	0
  #jihepos[ 1 , 4 ]<- 	0
  #jihepos[ 2 , 3 ]<- 	0
  ...
  jihedis<<-jihedis
  jihepos<<-jihepos
}#jihedis控制map的区块，jihepos控制区块之间交流的概率



#disper :near radius global
simu<-function(times,disper,ppool,halength=mapx){
  tim<-0
  qaq<-matrix(nrow = times,ncol = spn+1)
  colnames(qaq)<-0:spn
  while (tim<times) {tim<-tim+1
  
  for(xi in 1:mapx){
    for (yj in 1:mapy) {
      idk<-themap[yj,xi]
      if(idk>0){
        jiheid<-jihedis[yj,xi]
        suit<-msp[[idk]][yj,xi]
        if(rbinom(1,1,suit)!=1){
          if(rbinom(1,1,ppool)==1){
            ...
            if(disper=="near"){
              ...
              fec_sp<-NULL
              for (feci in fii) {
                if(jihedis[yj,feci]==jiheid){
                  fec_sp<-c(fec_sp,themap[yj,feci])
                }}
              for (fecj in fjj) {
                if(jihedis[fecj,xi]==jiheid){
  ...
  hax1<-round((mapx-halength))/2+1 
  hax2<-round(mapx+halength)/2
  hay1<-round((mapy-halength))/2+1 
  hay2<-round(mapy+halength)/2    
...
for(replic in 1:50){
  for(enviro in 1:12){
    for(pool in c(0.0001,0.001,0.01,0.1,1)){
      ...
      inisimu(80,12,12,enviro)
        fragmap(1)
      simu(10000,dis=disper,ppool = pool,halength=12)
