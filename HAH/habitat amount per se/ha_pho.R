...
lunwen<-function(){
  mylunwen1<-"在测试divesity多样性指数对局域景观内栖息地总量变化的响应时设置了三个不同的扩散距离(D)，五个不同的迁入率(a)和五个不同的空间自相关程度，因此总共有75种组合。结果(下图)显示取样区域所在斑块大小与物种多样性无显著相关性的占比pnoef%（nnoef种参数组合，p ≥ 0.05）正相关占比为pposi% (nposi种参数组合，p<0.05)，负相关占比为pneg% (nneg种参数组合，p<0.05)，单峰曲线占比为pgao%（ngao种参数组合）。这表明大多数情况是支持栖息地总量假说的。"
...
file_name<-"/Users/jingwufeng/Downloads/frag_test/hamean0918.csv"
profragdata<-read.csv(file_name)#habitat amount
...
profragdata$patch=profragdata$patch^2
...
for (diver in c("sp","shannon","simpson" ,"invsimpson" )) {
  ...
  #write.csv(modeldf,paste0("model",diver,"model-ha.csv"),row.names = F)
...
{
  jpgfile<-paste0("pho_v/ha",diver,".png")
  png(jpgfile,width = 2400,height = 2400)
  sheji<-F
  title<-"Habitat amount in the local landscape"
