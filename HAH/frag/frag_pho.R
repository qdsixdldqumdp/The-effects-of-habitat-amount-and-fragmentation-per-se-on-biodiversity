...
lunwen<-function(){
  mylunwen1<-"在测试divesity多样性指数对纯破碎化程度变化的响应时设置了三个不同的扩散距离(D)，五个不同的迁入率(a)和五个不同的空间自相关程度，因此总共有75种组合。结果(下图)显示取样区域所在斑块大小与物种多样性无显著相关性的占比pnoef%（nnoef种参数组合，p ≥ 0.05）正相关占比为pposi% (nposi种参数组合，p<0.05)，负相关占比为pneg% (nneg种参数组合，p<0.05)，单峰曲线占比为pgao%（ngao种参数组合）。这表明大多数情况是支持栖息地总量假说的。"
...
for (diver in c("sp","shannon","simpson" ,"invsimpson" )) {
  ...
  for (disper in c("near","radius","global")) {
    for(enviro in c(1,3,6,9,12)){
      ...
      xzhiyu<-log(max(patch),2)-log(min(patch),2)
      ...
      plotsize<-10
      p<-
        ggplot(new_data,aes(x=log(patch,2),y=sp,group=poolposible,color=poolposible))+
        geom_point(size=plotsize/4) +
        scale_color_brewer(palette="Dark2")+
        geom_line(data = new_data2,aes(x=log(patch,2),y=sp,group=poolposible,color=poolposible),size=1)+
        ...
        scale_x_continuous(breaks = log(c(1,2,4,8,16,36,72),2) ,labels =if(letind%in%c(1:3*5)){ c("1","2","4","8","16","36","72")}else{NULL})+
  ...
  #write.csv(modeldf,"model-frag.csv",row.names = F)
